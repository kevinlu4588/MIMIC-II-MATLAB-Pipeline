function mimic_read_matFile(matFileFullPath, index)
...
for i = 1:numberCellsToProcess
        ...
        bp_sample = sample(2, :);
        ecg_sample = sample(3, :);
        %If sample is too short in length, lots of bugs occur(not enough
        %data points), so skipping these samples.
        %Most samples are > 40,000 data points
        if(length(ecg_sample) < 2000)
            continue;
        end
...
mimic_save_data(matFileFullPath, finalXData, finalYData, cellstr(finalIDData), numberCellsToProcess, index);
disp("finished!")
